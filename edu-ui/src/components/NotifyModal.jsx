...
const NotifyModal = () => {
    ...
    const handleDeleteAll = () => {
        ...
        if(window.confirm(`Bạn có ${newArr.length} thông báo chưa đọc. Bạn có chắc muốn xóa tất cả không?`)){
                        ...
                        <div key={index} className="px-2 mb-3" >
                            ...
                            onClick={() => handleIsRead(msg)}>
                                ...
                                <div className="mx-1 flex-fill">
                                    <div>
                                        <strong className="mr-1">{msg.user?.username || 'Hệ thống'}</strong>
                                        <span>{msg.text}</span>
                                    </div>
                                    {msg.content && <small>{msg.content.length > 80 ? `${msg.content.slice(0, 80)}...` : msg.content}</small>}
                                </div>

                                {
                                    msg.image && !msg.image.match(/raw\/upload|\.pdf|\.doc|\.docx|\.xls|\.xlsx|\.ppt|\.pptx/i) &&
