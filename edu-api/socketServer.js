...
const SocketServer = (socket) => {
  ...
  socket.on("recallMessage", (data) => {
    // data: { messageId, sender, recipient, conversationId, recipients }
    if (data.conversationId && Array.isArray(data.recipients)) {
      emitToUsers(socket, data.recipients.filter((id) => toId(id) !== toId(data.sender)), "recallMessageToClient", data);
      return;
    }
    socket.to(toId(data.recipient)).emit("recallMessageToClient", data);
  });
