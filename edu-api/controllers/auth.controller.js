  ...
  login: async (req, res) => {
    try {
      ...
      const user = await Users.findOne({ email: email.toLowerCase().trim() })
        ...
        .populate("friendRequests", "avatar username fullname followers following");
  ...
  generateAccessToken: async (req, res) => {
    try {
      ...
      const verifyToken = (token) => new Promise((resolve, reject) => {
        jwt.verify(token, process.env.REFRESH_TOKEN_SECRET, (refreshErr, refreshResult) => {
          if (!refreshErr) return resolve(refreshResult);

          jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (accessErr, accessResult) => {
            if (accessErr) return reject(accessErr);
            return resolve(accessResult);
          });
        });
      });

      verifyToken(rf_token).then(async (result) => {
