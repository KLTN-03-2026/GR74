...
const messageCtrl = {
    ...
    createMessage: async (req, res) => {
        try {
            ...
            let directRecipientId = recipient;
            if (conversationId && !directRecipientId) {
                const tempConv = await Conversations.findById(conversationId);
                if (tempConv && !tempConv.isGroup) {
                    directRecipientId = tempConv.recipients.find(id => id.toString() !== sender.toString());
                }
            }

            if (directRecipientId) {
                const userChecks = await Users.find({
                    _id: { $in: [sender, directRecipientId] }
                }).select("blockedUsers");

                const senderUser = userChecks.find(u => u._id.toString() === sender.toString());
                const recipientUser = userChecks.find(u => u._id.toString() === directRecipientId.toString());

                if (senderUser && senderUser.blockedUsers.includes(directRecipientId.toString())) {
                    return res.status(400).json({ msg: "Bạn đã chặn người dùng này." });
                }
                if (recipientUser && recipientUser.blockedUsers.includes(sender.toString())) {
                    return res.status(400).json({ msg: "Người dùng này đã chặn bạn." });
                }
            }
    ...
    recallMessage: async (req, res) => {
        try {
            const message = await Messages.findOne({_id: req.params.id, sender: req.user._id})
            if(!message) return res.status(404).json({msg: 'Tin nhắn không tồn tại hoặc không phải của bạn.'})
            if(message.recalled) return res.status(400).json({msg: 'Tin nhắn đã được thu hồi rồi.'})
            ...
            res.json({msg: 'Đã thu hồi tin nhắn!', message})
        } catch (err) {
            return res.status(500).json({msg: err.message})
        }
    },
